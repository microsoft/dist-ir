...
def test_horizontal_parallelism_followed_by_data_parallelism():
    h = Helper()
    xs = h.function.add_op(
        "Broadcast",
        "Broadcast/x",
    ...
    y = h.function.add_op(
        ...
        "Gather/y",
        inputs=[yis],
        attributes={"dim": 0, "device": h.devices[0]},
        output_names=["y"],
    )
    h.function = h.function.finalize()
    output_data = h.executor.compute(h.function, h.input_data)
...
if __name__ == "__main__":
    ...
    test_horizontal_parallelism_followed_by_data_parallelism()
